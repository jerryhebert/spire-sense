...
public class OverlayTextTests
{
    ...
    [Fact]
    public void TheThreePartsOfThePanelComeBackSeparately()
    {
        ...
        var advice = new DeckAdviceResult("Block", 0.4, HasData: true);

        var text = OverlayText.Build(analysis, true, CycleFigures.From(analysis, new RunStats()), advice);

        Assert.Contains("Spire Sense", text.Summary);
        Assert.Contains("weakest", text.Summary);
        Assert.DoesNotContain("weakest", text.Counts);
        Assert.Contains("Frontloaded", text.Counts);
        Assert.Contains("Per turn", text.PerTurn);
    }

    [Fact]
    public void ThePanelCarriesNoOverallScore()
    {
        // There was a 0-10 "deck power" number here, and briefly a forecast of what the next fight
        // would cost. Both are gone: what the panel says about the deck as a whole is the name of
        // its weakest area, and nothing else.
        var text = Render(new[] { CardFacts.Named("StrikeIronclad", CardKind.Attack) });
    ...
    [Fact]
    public void TheWeakestAreaIsNamedOnItsOwnLine()
    {
        // The panel is only as wide as its widest line, so this sharing the title's line would set
        // a floor on the whole panel that no amount of dragging the resize grip could get past.
        var analysis = DeckAnalysis.Analyze(new[] { CardFacts.Named("StrikeIronclad", CardKind.Attack) });
        var advice = new DeckAdviceResult("Acceleration", 0.3, HasData: true);

        var text = OverlayText.Build(analysis, true, CycleFigures.From(analysis, new RunStats()), advice).ToString();
        var lines = text.Split('\n');

        var titleLine = Assert.Single(lines, l => l.Contains("Spire Sense"));
        Assert.DoesNotContain("weakest", titleLine);
